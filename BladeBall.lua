...
local UserInputService = game:GetService("UserInputService")
local responses = {"lol", "??", "damn", "ez", "mad cuz bad", "skill issue", "cry"}
local gameEndResponses = {"ggs", "gg uwu", "good game", "ggs yall", "wp", "ggs man"}
...
local AutoParrys = Window:CreateTab("Others Auto Parry", 13014537525)
...
local AutoParryToggle = AutoParry:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            startAutoParry()
            notify("Auto Parry", "Auto Parry Enabled", 1)
        else
            stopAutoParry()
            notify("Auto Parry", "Auto Parry Disabled", 1)
        end
    end,
})

local AutoRagingDeflect = AutoParry:CreateToggle({
    Name = "Auto Rage Parry/Rapture Parry (Need The Real Skill)",
    CurrentValue = false,
...
local ToggleParryOff = AutoParry:CreateKeybind({
   Name = "Toggle Parry Disable (Bind)",
   CurrentKeybind = "Two",
   HoldToInteract = false,
   Flag = "ToggleParryOff",
   Callback = function(Keybind)
...
AutoParrys:CreateButton({
    Name = "Visual God Premium Cracked 🤣 (Rejoin To Disable)",
    Callback = function()
